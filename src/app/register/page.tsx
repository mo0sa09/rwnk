...
export default function RegisterPage() {
  ...
  const inp:React.CSSProperties={width:'100%',height:44,background:'#FAFAFA',border:`1px solid ${BR}`,borderRadius:10,padding:'0 12px',fontSize:13,color:T1,outline:'none',fontFamily:"var(--font-tajawal),'Segoe UI',Tahoma,'Geeza Pro',Arial,sans-serif",transition:'all .2s'}
  ...
  if (checking) return (
    <div style={{ height:'100vh', display:'flex', alignItems:'center', justifyContent:'center', fontFamily:"var(--font-tajawal),'Segoe UI',Tahoma,'Geeza Pro',Arial,sans-serif", color:T3, fontSize:14 }}>
      جاري التحميل...
    </div>
  )
  ...
  return (
    <div className="auth-shell" style={{width:'100vw',height:'100vh',overflow:'hidden',display:'grid',gridTemplateColumns:'1fr 1fr',fontFamily:"var(--font-tajawal),'Segoe UI',Tahoma,'Geeza Pro',Arial,sans-serif",direction:'rtl'}}>
      <div className="auth-brand-panel" style={{background:T1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:48,position:'relative',overflow:'hidden'}}>
        ...
        <div style={{position:'relative',zIndex:1,textAlign:'center'}}>
          <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:12,marginBottom:36}}>
            <Image src="/logo-icon.png" alt="رَوْنَق" width={36} height={43} style={{width:36,height:43,objectFit:'contain',flexShrink:0}}/>
          ...
          <p style={{fontSize:13,color:'rgba(255,255,255,0.45)',lineHeight:1.6}}>دليلك الشامل نحو منزل بمستوى الفنادق</p>
        </div>
      </div>
      <div className="auth-form-panel" style={{background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',padding:'28px 52px',backgroundImage:'radial-gradient(circle,rgba(103,71,178,0.04) 1px,transparent 1px)',backgroundSize:'28px 28px'}}>
        <div style={{width:'100%',maxWidth:340}}>
          ...
          {!success&&<form onSubmit={handle}>
            ...
            <button type="submit" disabled={loading} style={{width:'100%',height:46,background:loading?'#8b6dd4':P,color:'#fff',border:'none',borderRadius:11,fontSize:14,fontWeight:900,cursor:loading?'wait':'pointer',fontFamily:"var(--font-tajawal),'Segoe UI',Tahoma,'Geeza Pro',Arial,sans-serif",boxShadow:'0 2px 12px rgba(103,71,178,.28)'}}>
          ...
          {!success&&<button onClick={signInWithGoogle} style={{width:'100%',height:42,background:'#fff',border:`1px solid ${BR}`,borderRadius:11,fontSize:13,fontWeight:700,color:T2,display:'flex',alignItems:'center',justifyContent:'center',gap:9,cursor:'pointer',fontFamily:"var(--font-tajawal),'Segoe UI',Tahoma,'Geeza Pro',Arial,sans-serif"}}>
