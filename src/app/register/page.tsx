...
export default function RegisterPage() {
  ...
  async function handle(e:React.FormEvent){
    ...
    try{await signUp(form.email,form.password,`${form.firstName} ${form.lastName}`.trim(),form.phone);window.location.href='/checkout'}
    catch(err:any){setError(err.message?.includes('already')?'هذا البريد مسجّل مسبقاً':'حدث خطأ')}
  ...
  return (
    <div style={{width:'100vw',height:'100vh',overflow:'hidden',display:'grid',gridTemplateColumns:'1fr 1fr',fontFamily:"'Th','Noto Kufi Arabic',serif",direction:'rtl'}}>
      <div style={{background:T1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',padding:48,position:'relative',overflow:'hidden'}}>
      ...
      <div style={{background:'#fff',display:'flex',alignItems:'center',justifyContent:'center',padding:'28px 52px',backgroundImage:'radial-gradient(circle,rgba(103,71,178,0.04) 1px,transparent 1px)',backgroundSize:'28px 28px'}}>
        <div style={{width:'100%',maxWidth:340}}>
          ...
          {error&&<p style={{fontSize:12,color:'#A32D2D',marginBottom:12,padding:'8px 12px',background:'#FEF2F2',borderRadius:8}}>{error}</p>}
          <form onSubmit={handle}>
            <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:10,marginBottom:10}}>
              <div><label style={{display:'block',fontSize:10,fontWeight:700,color:T2,textTransform:'uppercase',letterSpacing:0.4,marginBottom:5}}>الاسم الأول *</label><input placeholder="نورة" value={form.firstName} onChange={set('firstName')} onFocus={focus} onBlur={blur} style={inp}/></div>
              <div><label style={{display:'block',fontSize:10,fontWeight:700,color:T2,textTransform:'uppercase',letterSpacing:0.4,marginBottom:5}}>الاسم الأخير</label><input placeholder="العنزي" value={form.lastName} onChange={set('lastName')} onFocus={focus} onBlur={blur} style={inp}/></div>
            </div>
            {[{k:'email',l:'البريد الإلكتروني *',t:'email',ph:'example@email.com',ltr:true},{k:'phone',l:'الجوال',t:'tel',ph:'+965 XXXX XXXX',ltr:true},{k:'password',l:'كلمة المرور *',t:'password',ph:'8 أحرف على الأقل',ltr:true}].map(f=>(
              <div key={f.k} style={{marginBottom:10}}>
                <label style={{display:'block',fontSize:10,fontWeight:700,color:T2,textTransform:'uppercase',letterSpacing:0.4,marginBottom:5}}>{f.l}</label>
                <input type={f.t} placeholder={f.ph} dir={f.ltr?'ltr':'rtl'} value={form[f.k as keyof typeof form]} onChange={set(f.k)} onFocus={focus} onBlur={blur} style={inp}/>
              </div>
            ))}
          ...
          <button onClick={signInWithGoogle} style={{width:'100%',height:42,background:'#fff',border:`1px solid ${BR}`,borderRadius:11,fontSize:13,fontWeight:700,color:T2,display:'flex',alignItems:'center',justifyContent:'center',gap:9,cursor:'pointer',fontFamily:"'Th',serif"}}>
            <GSVG/> التسجيل بحساب Google
          </button>
        </div>
      </div>
    </div>
