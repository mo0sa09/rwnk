...
import {
  ...
  IconShieldCheck, IconFileText, IconBolt,
  IconLock, IconUser,
} from '@tabler/icons-react'

type Step = 'loading' | 'success' | 'done'
...
export default function SuccessPage() {
  ...
  useEffect(() => {
    async function load() {
      ...
      const urlPurchId = params.get('purchaseId')
      const raw        = sessionStorage.getItem('rwnq_purchase')
      const session    = raw ? JSON.parse(raw) : null
      const purchaseId = urlPurchId ?? session?.purchaseId

      if (purchaseId && (session?.email || urlPurchId)) {
        try {
          const { createClient } = await import('@supabase/supabase-js')
          const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!) as any
          const { data: pu } = await sb.from('purchases')
            .select('id,invoice_number,amount,status,downloads_limit,downloads_used,email').eq('id', purchaseId).single()
          if (pu) {
            setInfo({ ...session, purchaseId: pu.id, invoice: pu.invoice_number, downloadsLimit: pu.downloads_limit??5, downloadsUsed: pu.downloads_used??0, email: pu.email??session?.email })
          } else {
            setInfo({ ...session, invoice: 'RWN-2025-' + Math.floor(Math.random()*9000+1000), downloadsLimit:5, downloadsUsed:0 })
          }
        } catch {
          setInfo({ ...session, invoice: 'RWN-2025-' + Math.floor(Math.random()*9000+1000), downloadsLimit:5, downloadsUsed:0 })
        }
  ...
  async function handleDownload() {
    if (!info?.purchaseId) { alert('رقم الطلب غير موجود'); return }
    setDlLoad(true)
    try {
      const { createClient } = await import('@supabase/supabase-js')
      const sb = createClient(process.env.NEXT_PUBLIC_SUPABASE_URL!, process.env.NEXT_PUBLIC_SUPABASE_ANON_KEY!) as any
      const { data: { user } } = await sb.auth.getUser()
      const { data: tokenRow, error } = await sb.from('download_tokens')
        .insert({ purchase_id: info.purchaseId, user_id: user?.id ?? null }).select().single()
      if (error) throw new Error('LIMIT_REACHED')
      window.open(`/api/download?token=${tokenRow.token}`, '_blank')
      setInfo((p: any) => ({ ...p, downloadsUsed: (p.downloadsUsed??0) + 1 }))
    } catch (err: any) {
  ...
  async function handleCreateAccount(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await createAccountAfterPurchase(info?.email ?? '', info?.purchaseId ?? '')
      if (result.alreadyExists) setAldy(true)
      setStep('done')
    } catch (err: any) { setPwError(err.message ?? 'حدث خطأ') }
    finally { setPwLoad(false) }
  }

  const dlLeft = (info?.downloadsLimit ?? 5) - (info?.downloadsUsed ?? 0)
  ...
  return (
    <>
      ...
      <main style={{ minHeight:'100vh', display:'flex', alignItems:'center', justifyContent:'center', paddingTop:68, paddingBottom:40, fontFamily:"'Th','Noto Kufi Arabic',serif", direction:'rtl', background:'#fff', backgroundImage:'radial-gradient(ellipse 70% 55% at 50% 30%, rgba(54,219,156,0.05) 0%, transparent 70%)' }}>
        <div style={{ width:'100%', maxWidth:500, padding:'44px 40px', background:'#fff', border:`1px solid ${C.border}`, borderRadius:24, boxShadow:'0 4px 24px rgba(103,71,178,.07),0 16px 48px rgba(103,71,178,.05)' }}>
          ...
          {info && (
            <div style={{ background:C.primaryLight, border:`1px solid ${C.border}`, borderRadius:12, padding:'12px 16px', marginBottom:20 }}>
              {[
                { label:'رقم الطلب',     value: info.invoice ?? '—' },
                { label:'البريد',        value: info.email ?? '—'   },
                { label:'المبلغ المدفوع',value: '15 د.ك',  color: C.primary },
              ].map((r,i) => (
                <div key={i} style={{ display:'flex', justifyContent:'space-between', fontSize:12, marginBottom:i<2?7:0, direction:'rtl' }}>
                  <span style={{ color:C.text3 }}>{r.label}</span>
                  <span style={{ fontWeight:700, color: r.color ?? C.text1 }}>{r.value}</span>
                </div>
              ))}
            </div>
          )}
          ...
          <div style={{ background:C.surface, border:`1px solid ${C.border}`, borderRadius:14, padding:'16px 18px', marginBottom:18 }}>
            <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between', marginBottom:10 }}>
              ...
              <span style={{ fontSize:11, color:C.text3, background:'#fff', border:`1px solid ${C.border}`, padding:'3px 10px', borderRadius:999 }}>
                {dlLeft} من {info?.downloadsLimit ?? 5} تحميل متبقي
              </span>
            </div>
            {/* Progress bar */}
            <div style={{ height:4, background:C.border, borderRadius:999, marginBottom:14, overflow:'hidden' }}>
              <div style={{ height:'100%', width:`${((info?.downloadsUsed??0)/(info?.downloadsLimit??5))*100}%`, background:C.primary, borderRadius:999, transition:'width .4s' }} />
            </div>
            <button onClick={handleDownload} disabled={dlLoad || dlLeft <= 0} style={{
            ...
            <div style={{ border:`1px solid ${C.border}`, borderRadius:14, padding:'16px 18px', marginBottom:16 }}>
              ...
              <form onSubmit={handleCreateAccount}>
                <div style={{ marginBottom:10 }}>
                  ...
                  <input type="password" placeholder="8 أحرف على الأقل" value={pwd} onChange={e=>setPwd(e.target.value)} onFocus={focus} onBlur={blur} style={inpS} />
                </div>
                <div style={{ marginBottom:12 }}>
                  <label style={{ display:'block', fontSize:10, fontWeight:700, color:C.text2, textTransform:'uppercase', letterSpacing:0.4, marginBottom:5 }}>تأكيد كلمة المرور</label>
                  <input type="password" placeholder="••••••••" value={pwd2} onChange={e=>setPwd2(e.target.value)} onFocus={focus} onBlur={blur} style={inpS} />
                </div>
                {pwError && <p style={{ fontSize:12, color:'#A32D2D', marginBottom:10, padding:'7px 12px', background:'#FEF2F2', borderRadius:8 }}>{pwError}</p>}
                <button type="submit" disabled={pwLoading} style={{ width:'100%', height:44, background:pwLoading?'#8b6dd4':C.primary, color:'#fff', border:'none', borderRadius:11, fontSize:13, fontWeight:900, cursor:pwLoading?'wait':'pointer', fontFamily:"'Th',serif", display:'flex', alignItems:'center', justifyContent:'center', gap:8 }}>
                  <IconUser size={16} />
